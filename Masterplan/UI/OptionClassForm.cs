...
namespace Masterplan.UI
{
	internal class OptionClassForm : Form
	{
		...
		public Masterplan.Data.Class Class
		{
			get
			{
				return this.fClass;
		...
		public OptionClassForm(Masterplan.Data.Class c)
		{
		...
		{
			ListViewItem grayText = this.LevelList.Items.Add(ld.ToString());
			grayText.Tag = ld;
			if (ld.Count == 0)
			{
			...
			{
				grayText.Group = this.LevelList.Groups[0];
				return;
			}
		...
		private void EditBtn_Click(object sender, EventArgs e)
		{
			...
			{
				int level = this.fClass.Levels.IndexOf(this.SelectedLevel);
				bool flag = level == -1;
				...
				if (optionLevelForm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
				{
					if (!flag)
					{
						this.fClass.Levels[level] = optionLevelForm.Level;
					}
					else
					{
		...
		private void update_levels()
		{
			...
			foreach (LevelData level in this.fClass.Levels)
			{

...
namespace Masterplan.UI
{
	internal class DisplayNameForm : Form
	{
		...
		private void Application_Idle(object sender, EventArgs e)
		{
			...
			this.SetNameBtn.Enabled = (!(this.NameBox.Text != "") || this.SelectedCombatant == null ? false : this.NameBox.Text != this.SelectedCombatant.DisplayName);
		}
		...
		private void InitializeComponent()
		{
			...
			this.CombatantList.Columns.AddRange(new ColumnHeader[] { this.CombatantHdr });
			...
			base.ClientSize = new System.Drawing.Size(612, 368);
		...
		private void update_list()
		{
			...
			foreach (CombatData fCombatant in this.fCombatants)
			{
				ListViewItem listViewItem = this.CombatantList.Items.Add(fCombatant.DisplayName);
				listViewItem.Tag = fCombatant;
			}
		}
		...
		private void update_map_area()
		{
			...
			if (this.SelectedCombatant != null && this.SelectedCombatant.Location != CombatData.NoPoint)
			{
				...
				int y = this.SelectedCombatant.Location.Y - num1;
				int num2 = num + size + num;
				int num3 = num1 + size + num1;
				empty = new Rectangle(x, y, num2, num3);
			}
			else if (this.fEncounter.MapAreaID != Guid.Empty)
			{
				...
				if (mapArea != null)
				{
					empty = mapArea.Region;
				}
			}
			this.Map.Viewpoint = empty;
		}
		...
		private void update_stat_block()
		{
			...
			EncounterCard encounterCard = card;
			this.Browser.DocumentText = HTML.StatBlock(encounterCard, this.SelectedCombatant, this.fEncounter, true, false, true, CardMode.View, DisplaySize.Small);
		}
	}
}
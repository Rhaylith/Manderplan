...
namespace Masterplan.UI
{
	internal class TrapAttackForm : Form
	{
		...
		{
			int num = 2;
			int num1 = this.fLevel + 5;
			int num2 = this.fLevel + 3;
			if (this.fElite)
			{
				num += 2;
				num1 += 2;
			...
			ListViewItem item = this.AdviceList.Items.Add("Initiative");
			item.SubItems.Add(string.Concat("+", num));
			item.Group = this.AdviceList.Groups[0];
			ListViewItem listViewItem = this.AdviceList.Items.Add("Attack vs AC");
			listViewItem.SubItems.Add(string.Concat("+", num1));
			listViewItem.Group = this.AdviceList.Groups[1];
			ListViewItem item1 = this.AdviceList.Items.Add("Attack vs other defence");
			item1.SubItems.Add(string.Concat("+", num2));
			item1.Group = this.AdviceList.Groups[1];
		}
	}
}
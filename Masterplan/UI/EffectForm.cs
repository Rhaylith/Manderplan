...
namespace Masterplan.UI
{
	internal class EffectForm : Form
	{
		...
		public EffectForm(OngoingCondition condition, Encounter enc, CombatData current_actor, int current_round)
		{
			...
			foreach (string str in Conditions.GetConditions())
			{
				this.ConditionBox.Items.Add(str);
			}
			foreach (DamageType value in Enum.GetValues(typeof(DamageType)))
			{
				this.DamageTypeBox.Items.Add(value);
				this.DamageModTypeBox.Items.Add(value);
			}
			...
			foreach (EncounterSlot slot in enc.Slots)
			{
				...
				{
					this.DurationCreatureBox.Items.Add(combatDatum);
				}
			}
			...
			{
				this.DurationCreatureBox.Items.Add(trap);
			}
		...
		private void Application_Idle(object sender, EventArgs e)
		{
			...
			this.DamageModValueLbl.Enabled = (!this.DamageModBtn.Checked ? false : this.DamageModDirBox.SelectedItem.ToString() != "Immune");
			this.DamageModValueBox.Enabled = (!this.DamageModBtn.Checked ? false : this.DamageModDirBox.SelectedItem.ToString() != "Immune");
			...
			string selectedItem = this.DurationBox.SelectedItem as string;
			this.DurationCreatureBox.Enabled = (selectedItem == "Lasts until the start of someone's next turn" ? true : selectedItem == "Lasts until the end of someone's next turn");
			this.CreatureLbl.Enabled = this.DurationCreatureBox.Enabled;
			this.ModBox.Enabled = selectedItem == "Save ends";
		...
		private void EffectTypeChanged(object sender, EventArgs e)
		{
			...
			foreach (RadioButton radioButton1 in radioButtons)
			{
			...
			int height = base.Height - this.DurationGroup.Bottom;
			base.Height = top + this.ConditionPanel.Height + this.DamagePanel.Height + this.DefencePanel.Height + this.DamageModPanel.Height + this.RegenPanel.Height + this.AuraPanel.Height + num + this.DurationGroup.Height + height;
		}
		...
		private Guid get_id(object obj)
		{
			...
			{
				return (obj as CombatData).ID;
			}
			if (obj is Hero)
			{
				return (obj as Hero).ID;
			}
			if (!(obj is Trap))
			{
				return Guid.Empty;
			}
			return (obj as Trap).ID;
		}

		private object get_item(Guid id)
		{
			object obj;
			IEnumerator enumerator = this.DurationCreatureBox.Items.GetEnumerator();
			try
			{
				while (enumerator.MoveNext())
				{
					object current = enumerator.Current;
					if (current is CombatData && (current as CombatData).ID == id)
					{
						obj = current;
						return obj;
					}
					else if (!(current is Hero) || !((current as Hero).ID == id))
					{
						if (!(current is Trap) || !((current as Trap).ID == id))
						{
							continue;
						}
						obj = current;
		...
		private void InitializeComponent()
		{
			...
			modBox.Maximum = new decimal(numArray);
			NumericUpDown num = this.ModBox;
			int[] numArray1 = new int[] { 20, 0, 0, -2147483648 };
			...
			NumericUpDown defenceModBox = this.DefenceModBox;
			int[] numArray2 = new int[] { 100, 0, 0, -2147483648 };
			defenceModBox.Minimum = new decimal(numArray2);
			...
			NumericUpDown auraRadiusBox = this.AuraRadiusBox;
			int[] numArray3 = new int[] { 20, 0, 0, 0 };
			auraRadiusBox.Maximum = new decimal(numArray3);
			NumericUpDown numericUpDown = this.AuraRadiusBox;
			int[] numArray4 = new int[] { 1, 0, 0, 0 };
			numericUpDown.Minimum = new decimal(numArray4);
			...
			NumericUpDown auraRadiusBox1 = this.AuraRadiusBox;
			int[] numArray5 = new int[] { 1, 0, 0, 0 };
			auraRadiusBox1.Value = new decimal(numArray5);
			...
			base.ClientSize = new System.Drawing.Size(393, 659);
		...
		private void OKBtn_Click(object sender, EventArgs e)
		{
			...
			int value = (int)this.DamageModValueBox.Value;
			switch (this.DamageModDirBox.SelectedIndex)
			{
				case 0:
				{
					this.fCondition.DamageModifier.Value = value * -1;
					break;
				}
				case 1:
				{
					this.fCondition.DamageModifier.Value = value;
					break;
				}
				case 2:
				{
			...
			int num = (int)this.AuraRadiusBox.Value;
			this.fCondition.Aura.Details = string.Concat(num, ": ", this.AuraDetailsBox.Text);
			string selectedItem = this.DurationBox.SelectedItem as string;
			if (selectedItem == "Lasts until the end of the encounter")
			{
				this.fCondition.Duration = DurationType.Encounter;
			}
			else if (selectedItem == "Save ends")
			{
				this.fCondition.Duration = DurationType.SaveEnds;
				this.fCondition.SavingThrowModifier = (int)this.ModBox.Value;
			}
			else if (selectedItem == "Lasts until the start of someone's next turn")
			{
			...
			else if (selectedItem == "Lasts until the end of someone's next turn")
			{
				this.fCondition.Duration = DurationType.EndOfTurn;
				this.fCondition.DurationCreatureID = this.get_id(this.DurationCreatureBox.SelectedItem);
			}
			if (selectedItem == "Lasts until the start of someone's next turn" || selectedItem == "Lasts until the end of someone's next turn")
			{
				...
				{
					OngoingCondition durationRound = this.fCondition;
